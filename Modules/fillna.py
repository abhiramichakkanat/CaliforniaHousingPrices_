...
class FillNa:
    ...
    def __init__(self, data, fillStrategy,**kwargs):
        '''
    ...
    def apply(self,**kwargs):
        ...
        for strategy, columns in self.fillStrategy.items():
            if strategy == 'mean':
                ...
                self.interpolation(columns,filleddata)
            elif strategy =='movingAverage':
                self.fillMovingAverage(columns,filleddata,**kwargs)
        return filleddata   

    def interpolation(self,columns,data):
        for i in columns:
            data[i].interpolate(inplace=True)
            

    def fillMovingAverage(self, columns, data,**kwargs):
        '''
        ...
        for i in columns:
            ...
            if data[i].dtype in ['float64', 'int64']:  
                # Apply rolling mean with the given window
